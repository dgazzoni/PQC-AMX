...
#include "frodokem_ref_matmul_4x.h"
...
#ifdef ALLOC_MMAP
    int16_t *A = A_sa_speed_4x;
#else
    const int n32 = 32 * ((PARAMS_N + 31) / 32);
    int16_t A[PARAMS_N * n32];
#endif

    amx_mul_add_sa_plus_e_4x(out, A, s, (const uint16_t **)e, PARAMS_N);
