...
__attribute__((constructor)) static void alloc_as_speed(void) {
    ...
    A_as_speed = MEMORY_ALLOC(PARAMS_N * n32 * sizeof(int16_t));
}
#endif
...
#else
    const int n32 = 32 * ((PARAMS_N + 31) / 32);
    int16_t A[PARAMS_N * n32];
#endif

    amx_mul_add_as_plus_e(out, A, s, e, PARAMS_N);
...
__attribute__((constructor)) static void alloc_sa_speed(void) {
    const int n32 = 32 * ((PARAMS_N + 31) / 32);
    A_sa_speed = MEMORY_ALLOC(PARAMS_N * n32 * sizeof(int16_t));
}
#endif
...
int frodo_mul_add_sa_plus_e_matmul_only(
    ...
    amx_mul_add_sa_plus_e(out, A, s, e, PARAMS_N);
