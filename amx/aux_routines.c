...
void amx_poly_mul_mod_65536_u16_flatten_middle_block(uint16_t xy[32], uint64_t xy_reg, uint64_t zi) {
    ...
    AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg) | EXTRH_COPY_ONLY_Z_ROW(3 - zi));
    AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg + 1) | EXTRH_COPY_ONLY_Z_ROW(2 + zi));

    AMX_MAC16(MAC16_VECTOR | MAC16_X_OFFSET(64 * (xy_reg + 1) - 2) | MAC16_Z_ROW(zi) | MAC16_Y_SKIP);

    for (uint64_t i = 4; i < 64; i += 4) {
        AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg) | EXTRH_COPY_ONLY_Z_ROW(i + 1 - zi));
        AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg + 1) | EXTRH_COPY_ONLY_Z_ROW(i + zi));

        AMX_EXTRH(extrh_y_args | EXTRH_REG(xy_reg) | EXTRH_Z_ROW(i + 3 - zi));
        AMX_EXTRH(extrh_y_args | EXTRH_REG(xy_reg + 1) | EXTRH_Z_ROW(i + 2 + zi));

        AMX_VECINT(VECINT_X_OFFSET(64 * (xy_reg + 1) - i) | VECINT_Y_OFFSET(64 * (xy_reg + 1) - i - 2) |
                   VECINT_Z_ROW(zi) | VECINT_ALU_MODE_Z_ADD_X_ADD_Y);
    }

    AMX_STZ(AMX_PTR(xy) | STZ_Z_ROW(zi));
}
...
void amx_poly_mul_mod_65536_u16_merge_first_and_last_blocks(uint16_t xy[32], uint64_t xy_reg, uint64_t zi,
    ...
    AMX_LDZ(AMX_PTR(xy) | LDZ_Z_ROW(zi));

    AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg) | EXTRH_COPY_ONLY_Z_ROW(3 - zi));

    AMX_MAC16(MAC16_VECTOR | MAC16_X_OFFSET(64 * (xy_reg + 1) - offset - 2) | MAC16_Z_ROW(zi) | MAC16_Y_SKIP);

    for (i = 2; i < 62 - offset; i += 4) {
        AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg) | EXTRH_COPY_ONLY_Z_ROW(i + 3 - zi));
        AMX_EXTRH(extrh_y_args | EXTRH_REG(xy_reg) | EXTRH_Z_ROW(i + 5 - zi));

        AMX_VECINT(VECINT_X_OFFSET(64 * (xy_reg + 1) - offset - i - 2) |
                   VECINT_Y_OFFSET(64 * (xy_reg + 1) - offset - i - 4) | VECINT_Z_ROW(zi) |
                   VECINT_ALU_MODE_Z_ADD_X_ADD_Y);
    }

    for (; i < 62; i += 4) {
        AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg) | EXTRH_COPY_ONLY_Z_ROW(i + 2 + zi));
        AMX_EXTRH(extrh_x_args | EXTRH_COPY_ONLY_REG(xy_reg + 1) | EXTRH_COPY_ONLY_Z_ROW(i + 3 - zi));

        AMX_EXTRH(extrh_y_args | EXTRH_REG(xy_reg) | EXTRH_Z_ROW(i + 4 + zi));
        AMX_EXTRH(extrh_y_args | EXTRH_REG(xy_reg + 1) | EXTRH_Z_ROW(i + 5 - zi));

        AMX_VECINT(VECINT_X_OFFSET(64 * (xy_reg + 2) - offset - i - 2) |
                   VECINT_Y_OFFSET(64 * (xy_reg + 2) - offset - i - 4) | VECINT_Z_ROW(zi) |
                   VECINT_ALU_MODE_Z_ADD_X_ADD_Y);
    }
...
void amx_poly_mul_mod_65536_u16_merge_first_and_last_blocks_x_n_plus_1(uint16_t xy[32], uint64_t xy_reg, uint64_t zi,
                                                                       uint64_t d_mod_32) {
    const uint64_t extrh_x_args = EXTRH_COPY_ONLY | EXTRH_COPY_ONLY_LANE_WIDTH_16_BIT;
    const uint64_t extrh_y_args = EXTRH_GENERIC | EXTRH_DESTINATION_Y | EXTRH_LANE_WIDTH_16_BIT;
    ...
    AMX_VECINT(VECINT_X_OFFSET(64 * (xy_reg + 1) - offset - 2) | VECINT_Z_ROW(zi) | VECINT_ENABLE_MODE(0) |
               VECINT_ENABLE_VALUE(5) | VECINT_ALU_MODE_Z_SUB_X_ADD_Y);
