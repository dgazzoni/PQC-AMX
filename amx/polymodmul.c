...
void amx_poly_mul_mod_65536_mod_x_d_plus_1_u16_32nx32n_coeffs(uint16_t xy[], const uint16_t x[], const uint16_t y[],
                                                              int d, int n) {
    uint64_t matint_args = MATINT_ALU_MODE_Z_SUB_X_MUL_Y;
    uint64_t matint_extra_args = MATINT_ENABLE_MASK_IS_Y | MATINT_X_Y_ENABLE_MODE(2) | MATINT_X_Y_ENABLE_VALUE(d % 32);
    ...
    for (j = 1; j < 6; j++) {
        ...
        AMX_MATINT(matint_args | MATINT_X_REG(6) | MATINT_Y_REG(j) | MATINT_Z_ROW(0) |
                   (j == n - 1 ? matint_extra_args : 0));
    }
    ...
    for (; j < n; j++) {
        ...
        AMX_MATINT(matint_args | MATINT_X_REG(6) | MATINT_Y_REG(6) | MATINT_Z_ROW(0) |
                   (j == n - 1 ? matint_extra_args : 0));
    }
    ...
    for (j = 2; j < 6; j++) {
        ...
        AMX_MATINT(matint_args | MATINT_X_REG(6) | MATINT_Y_REG(j) | MATINT_Z_ROW(1) |
                   (j == n - 1 ? matint_extra_args : 0));
    }
    ...
    for (; j < n; j++) {
        ...
        AMX_MATINT(matint_args | MATINT_X_REG(6) | MATINT_Y_REG(6) | MATINT_Z_ROW(1) |
                   (j == n - 1 ? matint_extra_args : 0));
    }
    ...
    for (i = 2; i < n - 1; i++) {
        ...
        for (j = i + 1; j < n; j++) {
            ...
            AMX_MATINT(matint_args | MATINT_X_REG(6) | MATINT_Y_REG(reg_Y) | MATINT_Z_ROW(parity) |
                       (j == n - 1 ? matint_extra_args : 0));
        }
    ...
    amx_poly_mul_mod_65536_u16_merge_first_and_last_blocks_x_n_plus_1(xy, 6, 1 - parity, d32);
}
